...
class ExtrasState extends MusicBeatState
{
	...
	override function create()
	{
		...
		**/
		...
		{
			addWeek(Main.extraSongs[i][0], i, Main.extraSongs[i][1], Main.extraSongs[i][2]);
			for (j in cast(Main.extraSongs[i][0], Array<Dynamic>))
		...
		var extrasTxt:FlxText = new FlxText(0, 0, 0, "Press F to go back to Freeplay.", 16);
	...
	public function addSong(songName:String, weekNum:Int, songCharacter:String, songColor:FlxColor)
	{
		...
		{ //*/
			songs.push(new FreeplayState.SongMetadata(songName, weekNum, songCharacter, songColor));
	...
	function changeDiff(change:Int = 0)
	{
        ...
        if (curDifficulty == 3){ //we aint adding erect so
            curDifficulty = 2;
        }
