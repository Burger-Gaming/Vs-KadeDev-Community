...
class AllInOneMainMenuState extends MusicBeatState
{
	...
	function startIntro()
	{
		...
		serverSideSprite = new FlxSprite(FlxG.width - 240).makeGraphic(240, FlxG.height, FlxColor.fromString("#2f3237"));
		...
		var kdcLogo = new FlxSprite(-450, -350).loadGraphic(Paths.image("menus/base/title/KDCLogo"));
		kdcLogo.setGraphicSize(Std.int(kdcLogo.width * .5));
		kdcLogo.antialiasing = true;
		add(kdcLogo);

	    disStartText = new FlxText(230, FlxG.height - 150);
		disStartText.text = "Press Enter To Start!";
		disStartText.font = Paths.font("whitneybold.otf");
		disStartText.size = 60;
